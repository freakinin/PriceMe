    ...
    static async run() {
      ...
      `;
            ...
            for (const product of products.rows) {
                try {
                    // 2. Use AI with Google Search Grounding
                    console.log(`Checking product ${product.id}: ${product.url}`);

                    // Pass URL directly to AI with Google Search Grounding
                    const analysis = await AIService.analyzeProduct(product.url);

                    if (!analysis || !analysis.price) {
                        console.log(`Skipping product ${product.id}: Could not extract price. AI Summary: ${analysis.ai_analysis_summary}`);
                        continue;
                    }
            ...
            `;
                        ...
                        // Only notify if the change is significant (> 1%) or it's a price drop
                        if (Math.abs(Number(changePercent)) > 1 || newPrice < oldPrice) {
               ...
               WHERE id = ${product.id}
             `;
                        console.log(`No price change for ${product.id}.`);
                    }
