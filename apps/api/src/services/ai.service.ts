...
export class AIService {
    private static getModel() {
        // Using gemini-2.5-flash which supports Google Search Grounding
    ...
    static async analyzeProduct(urlOrMarkdown: string): Promise<AnalysisResult> {
        ...
        if (isUrl) {
            prompt = `
              ...
              - If there is a price range, use the LOWEST price.
              
              Extract the following fields and return as JSON:
              1. **title**: The exact product title.
              2. **price**: Numeric value ONLY (e.g. 19.99). If strictly no price found, return 0.
              3. **currency**: ISO 4217 code (USD, EUR, etc.).
              4. **materials**: Array of strings (e.g., "Cotton", "Gold"). 
