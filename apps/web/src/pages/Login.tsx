    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-8 py-12 bg-background">
        ...
        <div className="w-full max-w-[360px] space-y-7">
          ...
          {googleError && (
            <div className="text-sm text-destructive bg-destructive/8 border border-destructive/20 p-3 rounded-lg">
              {googleError === 'google_cancelled' ? 'Sign-in was cancelled.' : 'Google sign-in failed. Please try again.'}
            </div>
          )}
          <button
            type="button"
            onClick={() => { window.location.href = `${API_BASE}/auth/google`; }}
            className="w-full h-10 flex items-center justify-center gap-2.5 rounded-md border border-warm-200 bg-card text-sm font-medium text-foreground hover:bg-warm-100 transition-colors"
          >
